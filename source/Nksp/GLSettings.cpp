...
extern void ApplyGLSettings(BOOL bForce)
{
  ...
  if (!sam_bGfxAPIVulkan) {
      ...
      // get new settings
      pse = GetGLSettings(da.da_strRenderer);
  }
  else {
#ifdef ADAPTER_VULKAN_INFO
      CDisplayAdapter& da = _pGfx->gl_gaAPI[2/*GAT_VK*/].ga_adaAdapter[_pGfx->gl_iCurrentAdapter];
      CPrintF(TRANS("Detected: %s - %s - %s\n"), da.da_strVendor, da.da_strRenderer, da.da_strVersion);
      // get new settings
      CDisplayAdapter& da_d3d = _pGfx->gl_gaAPI[_pGfx->gl_eCurrentAPI].ga_adaAdapter[_pGfx->gl_iCurrentAdapter];
      pse = GetGLSettings(da_d3d.da_strRenderer);
#else
      CDisplayAdapter& da = _pGfx->gl_gaAPI[_pGfx->gl_eCurrentAPI].ga_adaAdapter[_pGfx->gl_iCurrentAdapter];
      CPrintF(TRANS("Detected: Vulkan - %s - %s\n"), da.da_strRenderer, da.da_strVersion);
      // get new settings
      pse = GetGLSettings(da.da_strRenderer);
#endif  
  }
