...
extern ENGINE_API INDEX	sam_iAspectRatioSizeI;
extern ENGINE_API INDEX	sam_iAspectRatioSizeJ;
...
extern ENGINE_API INDEX	sam_bGfxAPIVulkan;	// ### Vulkan
...
//	MessageBox(NULL, strTemp, "CommandLine", MB_OK);
	...
	for(INDEX ii = 0; ii < 4; ii++)
	{
		...
		{
			case 0 :
				g_nmVER.PrintF("%s", strInsert.str_String);
				break;
			case 1 :
				g_nmCID.PrintF("%s", strInsert.str_String);
				break;
			case 2 :
				g_nmID.PrintF("%s", strInsert.str_String);
				break;
			case 3 :
				g_nmPW.PrintF("%s", strTemp.str_String);
				break;
		}
...
//������ ���� ����	//(DevPartner Bug Fix)(2005-01-14)
		...
		if (g_hGame==NULL) 
		{
			ThrowF_t("%s", (const char*)GetWindowsError(GetLastError()));
		}
...
//#endif
	...
	switch(g_iCountry)
	{
		case BRAZIL:
#if defined(PLATFORM_WIN32)
			SetClassLongPtr( _hwndMain, -14 /* GCL_HICON */, (LONG)LoadIcon( _hInstanceMain, (LPCTSTR)IDI_GUNSOFT ) );
#endif
			g_bNasTrans = TRUE;
			break;
	}
	...
	_pShell->DeclareSymbol("persistent INDEX sam_bBorderless;",   &sam_bBorderlessActive);
	_pShell->DeclareSymbol("persistent INDEX sam_iAspectRatioSizeI;", &sam_iAspectRatioSizeI);
	_pShell->DeclareSymbol("persistent INDEX sam_iAspectRatioSizeJ;", &sam_iAspectRatioSizeJ);
	...
	_pShell->DeclareSymbol("persistent INDEX ssam_bGfxAPIVulkan;",         &sam_bGfxAPIVulkan); // ### Vulkan
	...
	_pShell->DeclareSymbol("user void Quit(void);", reinterpret_cast<void*>(&QuitGame));

	_pShell->DeclareSymbol("persistent user INDEX sam_iVideoSetup;",     &sam_iVideoSetup);
	_pShell->DeclareSymbol("user void ApplyRenderingPreferences(void);", reinterpret_cast<void*>(&ApplyRenderingPreferences));
	_pShell->DeclareSymbol("user void Benchmark(void);", reinterpret_cast<void*>(&BenchMark));
	...
	if ( sam_iGfxAPI == GAT_D3D ){ // ### D3D
		StartNewMode( GAT_D3D, sam_iDisplayAdapter, sam_iScreenSizeI, sam_iScreenSizeJ,
								(enum DisplayDepth)sam_iDisplayDepth, IsFullScreen( sam_bFullScreenActive), sam_bBorderlessActive );
	}
	if ( sam_iGfxAPI == GAT_OGL ){ // ### OGL
		StartNewMode( GAT_OGL, sam_iDisplayAdapter, sam_iScreenSizeI, sam_iScreenSizeJ,
								(enum DisplayDepth)sam_iDisplayDepth, IsFullScreen( sam_bFullScreenActive), sam_bBorderlessActive );
	}
...
*/
	...
	nmCmdLine = EventInfo.ulParameter2;
#endif
		...
		while( PeekMessage( &msg, NULL, 0, 0, PM_REMOVE)) 
		{
			...
			if( msg.message==WM_SYSCOMMAND)
			{
				...
				case SC_MAXIMIZE:
					...
					// go to full screen
					StartNewMode( (GfxAPIType)sam_iGfxAPI, sam_iDisplayAdapter, sam_iScreenSizeI, sam_iScreenSizeJ,
						(enum DisplayDepth)sam_iDisplayDepth, TRUE , FALSE);
					ShowWindow( _hwndMain, SW_SHOWNORMAL);
...
int main(int argc, char** argv) {
    // Basic initialization for Unix platforms
    printf("LastChaos macOS Launcher starting...\n");
    
    // Convert argc/argv to a single command line string if needed
    // For now, we'll pass an empty string as lpCmdLine
    return SubMain(NULL, NULL, (char*)"", 0);
}
#endif
...
BOOL TryToSetDisplayMode( enum GfxAPIType eGfxAPI, INDEX iAdapter, PIX pixSizeI, PIX pixSizeJ,
													enum DisplayDepth eColorDepth, BOOL bFullScreenMode, BOOL bBorderLessMode,bool bCenter)
{
...
*/
	// try to set the mode
	BOOL bSuccess = TryToSetDisplayMode( eGfxAPI, iAdapter, pixSizeI, pixSizeJ, eColorDepth, bFullScreenMode , bBorderlessMode);
	...
	if( !bSuccess)
	{
		...
		for( INDEX iMode=0; iMode<ctDefaultModes; iMode++) {
			...
			bSuccess = TryToSetDisplayMode( eGfxAPI, iAdapter, pixSizeI, pixSizeJ, eColorDepth, bFullScreenMode, bBorderlessMode);
			if( bSuccess) break;
		}
		// if all failed
		if( !bSuccess) {
...
bool	Set_Window_Size()
{
	if(sam_bFullScreenActive != 1)
	{
		...
		TryToSetDisplayMode( (GfxAPIType)sam_iGfxAPI, sam_iDisplayAdapter, t_Width, t_Height, (enum DisplayDepth)sam_iDisplayDepth, sam_bFullScreenActive, sam_bBorderlessActive,false);
...
void GetWindowSizeFromTxt()
{
	...
	if(fs.is_open())
	{
		...
		INDEX data2 =0;
		INDEX data3 =0;
		fs >> data1 >> data2 >> data3;
		sam_iScreenSizeI = data1;
		sam_iScreenSizeJ = data2;
		sam_bFullScreenActive = data3;
...
void SaveWindowSizeToTxt()
{
	...
	if(fs.is_open())
	{
		...
		data3 = sam_bFullScreenActive;
		fs << data1<<" "<<data2<<" "<<data3;
		fs.close();
	}
}

void GetGfxAPIFromTxt()
{
	CTString strFullPath = _fnmApplicationPath.FileDir();
	CTFileName fileName = CTString(strFullPath+"Data\\etc\\GfxAPI.txt");
	...
	if(fs.is_open())
	{
		...
		if (data == 1 ) {  
		  sam_iGfxAPI = GAT_D3D;
		  sam_bGfxAPIVulkan = FALSE;  
		  CPrintF(TRANS("Nksp GfxAPI: DirextX 9\n"));
		}
	    else if (data == 0 ) {
		  sam_iGfxAPI = GAT_OGL;
		  sam_bGfxAPIVulkan = FALSE;  
		  CPrintF(TRANS("Nksp GfxAPI: OpenGL\n"));
		}
	    else if (data == 2 ) {
		  sam_iGfxAPI = GAT_D3D;
		  sam_bGfxAPIVulkan = TRUE;  
		  CPrintF(TRANS("Nksp GfxAPI: Vulkan\n"));
		}
		else {
			sam_iGfxAPI = GAT_D3D;
			sam_bGfxAPIVulkan = FALSE;  
			CPrintF(TRANS("Nksp GfxAPI: Force DirextX 9 API\n"));
			SaveGfxAPISizeToTxt();
		}
		fs.close();
	}
...
void SaveGfxAPISizeToTxt()
{
	...
	{
		INDEX data;
		if ((sam_iGfxAPI == GAT_D3D) && (sam_bGfxAPIVulkan == FALSE)) {  
		  data = 1;
		}
	    else if ((sam_iGfxAPI == GAT_D3D) && (sam_bGfxAPIVulkan == TRUE)) {  
		  data = 2;
		}
	    else if (sam_iGfxAPI == GAT_OGL ) {
		  data = 0;
		} else {
		  data = 1;
		}
