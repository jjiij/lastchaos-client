...
namespace dxvk {
  ...
  DxvkGpuQueryStatus DxvkQuery::accumulateQueryDataForGpuQueryLocked(
    ...
    switch (m_type) {
      ...
      default:
        Logger::err(str::format("Vulkan: Unhandled query type: ", m_type));
        return DxvkGpuQueryStatus::Invalid;
    }
  ...
  void DxvkGpuQueryAllocator::createQueryPool() {
    ...
    if (vk->vkCreateQueryPool(vk->device(), &info, nullptr, &queryPool)) {
      Logger::err(str::format("Vulkan: Failed to create query pool (", m_queryType, "; ", m_queryPoolSize, ")"));
      return;
    }
  ...
  Rc<DxvkGpuQuery> DxvkGpuQueryPool::allocQuery(VkQueryType type) {
    switch (type) {
      ...
      default:
        Logger::err(str::format("Vulkan: Unhandled query type: ", type));
        return nullptr;
    }
  }
