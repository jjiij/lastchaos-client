...
namespace dxvk {
  ...
  bool DxvkStateCache::readCacheFile() {
    ...
    if (!ifile) {
      Logger::warn("Vulkan: No state cache file found");
      return true;
    }
    ...
    if (!readCacheHeader(ifile, curHeader)) {
      Logger::warn("Vulkan: Failed to read state cache header");
      return false;
    }
    ...
    if (curHeader.version < 8 || curHeader.version == 16
     || curHeader.version > newHeader.version) {
      Logger::warn("Vulkan: State cache version not supported");
      return false;
    }

    // Notify user about format conversion
    if (curHeader.version != newHeader.version)
      Logger::warn(str::format("Vulkan: Updating state cache version to v", newHeader.version));
    ...
    Logger::info(str::format(
      "Vulkan: Read ", m_entries.size(),
      " valid state cache entries"));

    if (numInvalidEntries) {
      Logger::warn(str::format(
        "Vulkan: Skipped ", numInvalidEntries,
  ...
  std::ofstream DxvkStateCache::openCacheFileForWrite(bool recreate) const {
    ...
    if (recreate) {
      Logger::warn("Vulkan: Creating new state cache file");
