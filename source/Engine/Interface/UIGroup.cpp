...
void CUIGroup::AddMember( CMemberInfo MemberInfo )
{
	...
	//iter = find_if(m_vecMember.begin(), m_vecMember.end(), bind2nd(ptr_fun(FindObj), MemberInfo.m_strName));                   // ### bind2nd and ptr_fun removed in C++17
	iter = find_if(m_vecMember.begin(), m_vecMember.end(), bind(std::not_fn(FindObj), placeholders::_1, MemberInfo.m_strName) ); // ### Fix me!!!
